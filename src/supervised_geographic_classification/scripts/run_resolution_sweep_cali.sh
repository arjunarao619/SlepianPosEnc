#!/bin/bash
# California Housing Resolution Sweep Experiment
...
# Output directories
RESULTS_DIR="$ROOT_DIR/results/california_resolution_sweep"
mkdir -p "$RESULTS_DIR"
...
echo "=============================================="
echo "California Housing Resolution Sweep Experiment"
echo "=============================================="
...
for ARCH in "${ARCHS[@]}"; do
    ...
    for MIN_R in "${MIN_RADII[@]}"; do
        ...
        if ! python "$PARENT_DIR/run_baselines_californiahousing.py" \
            --encoders "$ENCODERS" \
            --arch "$ARCH" \
