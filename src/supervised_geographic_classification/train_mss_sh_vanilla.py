...
def evaluate_coverage_curve(
    ...
    """
    Evaluate R² at different coverage fractions.

    For each coverage fraction f, we find the radius that covers f% of test points
    (by distance from cap center) and compute R² on those points.

    Args:
        model: Trained model
        test_coords: Test coordinates [N, 2]
        test_targets: Test targets (normalized) [N]
    ...
    for frac in coverage_fractions:
        ...
        targets = test_targets[covered_idx]
        ...
        results['rmse_values'].append(rmse_raw)
