...
# create input encoder based on model's encoding type
...
if input_enc in ['slepian', 'slepian_env']:
    slepian_raster = datasets.load_slepian(train_params['params'])
else:
    slepian_raster = None
...
locs_enc = enc.encode(locs, normalize=True).to(eval_params['device'])
with torch.no_grad():
