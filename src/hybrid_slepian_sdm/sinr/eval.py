...
class EvaluatorSNT:
    ...
    def run_evaluation(self, model, enc):
        ...
        loc_feat = enc.encode(obs_locs, normalize=True)
...
class EvaluatorGeoPrior:
    ...
    def run_evaluation(self, model, enc):
        ...
        for bb_id, bb in enumerate(range(len(batch_start)-1)):
            ...
            loc_feat = enc.encode(obs_locs_batch, normalize=True)
...
class EvaluatorGeoFeature:
    ...
    def get_split_feats(self, model, enc, split_ids, split_of_interest):
        ...
        locs_enc = enc.encode(locs, normalize=True)
        with torch.no_grad():
...
def launch_eval_run(overrides):
    ...
    input_enc = train_params['params']['input_enc']

    # Load environmental raster if needed
    if input_enc in ['env', 'sin_cos_env', 'slepian_env', 'slepian_direct_env']:
        raster = datasets.load_env().to(eval_params['device'])
    else:
        raster = None

    # Load Slepian raster if needed (for interpolation-based encoding)
    if input_enc in ['slepian', 'slepian_env']:
        slepian_raster = datasets.load_slepian(train_params['params']).to(eval_params['device'])
    else:
        slepian_raster = None

    # Load Slepian coefficients if needed (for direct computation)
    if input_enc in ['slepian_direct', 'slepian_direct_env']:
        slepian_data = datasets.load_slepian_direct(train_params['params'])
    else:
        slepian_data = None

    # Get sh_L from saved params (default to 10 for backward compatibility)
    sh_L = train_params['params'].get('sh_L', 10)
    enc = utils.CoordEncoder(input_enc, raster=raster, L=sh_L,
